...
<br/>
	...
	if(isset($_GET['target'])||$_GET['target'!=""]){$kategori="kategori='nike'";}else{$kategori="";}
	...
	$tampil= mysqli_query($link,"SELECT * FROM produk where $kategori ORDER by id_produk DESC LIMIT $offset,$dataPerPage");
	...
	while($isi=mysqli_fetch_assoc($tampil)){
      ...
      <div class="paging">
		...
		$query  = "SELECT COUNT(*) AS jumData FROM produk where $kategori";
		...
		for($page = 1; $page <= $jumPage; $page++)
		{
		         if ((($page >= $noPage - 3) && ($page <= $noPage + 3)) || ($page == 1) || ($page == $jumPage))
		         {
		            ...
		            else echo "<a href='".$_GET['target'].".php?target=".$_GET['target']."&page=".$page."'>".$page."</a> ";
		            $showPage = $page;
		         }
		}
