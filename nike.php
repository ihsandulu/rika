...
<div>
  <h2 class="page_title"><font color="#0033CC">Tas Nike</font></h2></div>
<br/>
    <?php 
	...
	$tampil= mysql_query("SELECT * FROM produk where kategori='nike' ORDER by id_produk DESC LIMIT $offset,$dataPerPage");
	...
	while($isi=mysql_fetch_assoc($tampil)){
      ...
      <div class="paging">
		...
		$query  = "SELECT COUNT(*) AS jumData FROM produk where kategori='nike'";
		...
		if ($noPage > 1) echo "<a href='produk.php?target=nike&page=".($noPage-1)."' id='prev'>Prev&nbsp;</a>";
		...
		for($page = 1; $page <= $jumPage; $page++)
		{
		         if ((($page >= $noPage - 3) && ($page <= $noPage + 3)) || ($page == 1) || ($page == $jumPage))
		         {
		            ...
		            else echo "<a href='nike.php?target=nike&page=".$page."'>".$page."</a> ";
		            $showPage = $page;
		         }
		}
		//menampilkan link Next
		if ($noPage < $jumPage) echo "<a href='produk.php?target=nike&page=".($noPage+1)."' id='next'>Next</a>";
		?>
		</div>