...
<br/>
	...
	$tampil= mysqli_query($link,"SELECT * FROM produk where kategori='adidas' ORDER by id_produk DESC LIMIT $offset,$dataPerPage");
	//$query=mysqli_query("SELECT * FROM barang where kategori='musical' order by id limit 0,12");
	...
	while($isi=mysqli_fetch_assoc($tampil)){
      ...
      <div class="paging">
		...
		if ($noPage > 1) echo "<a href='produk2.php?target=adidas2&page=".($noPage-1)."' id='prev'>Prev&nbsp;</a>";
		...
		for($page = 1; $page <= $jumPage; $page++)
		{
		         if ((($page >= $noPage - 3) && ($page <= $noPage + 3)) || ($page == 1) || ($page == $jumPage))
		         {
		            ...
		            else echo "<a href='adidas2.php?target=adidas2&page=".$page."'>".$page."</a> ";
		            $showPage = $page;
		         }
		}
		//menampilkan link Next
		if ($noPage < $jumPage) echo "<a href='produk2.php?target=adidas2&page=".($noPage+1)."' id='next'>Next</a>";
		?>
		</div>