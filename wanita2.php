...
<div><h2 class="page_title"><font color="#0033CC">Tas Wanita</font></h2></div>
<br/>
    <?php 
	...
	$tampil= mysqli_query($link,"SELECT * FROM produk where kategori='wanita' ORDER by id_produk DESC LIMIT $offset,$dataPerPage");
	...
	while($isi=mysqli_fetch_assoc($tampil)){
      ...
      <div class="paging">
		...
		$query  = "SELECT COUNT(*) AS jumData FROM produk where kategori='wanita'";
		...
		if ($noPage > 1) echo "<a href='produk2.php?target=wanita2&page=".($noPage-1)."' id='prev'>Prev&nbsp;</a>";
		...
		for($page = 1; $page <= $jumPage; $page++)
		{
		         if ((($page >= $noPage - 3) && ($page <= $noPage + 3)) || ($page == 1) || ($page == $jumPage))
		         {
		            ...
		            else echo "<a href='wanita2.php?target=wanita2&page=".$page."'>".$page."</a> ";
		            $showPage = $page;
		         }
		}
		//menampilkan link Next
		if ($noPage < $jumPage) echo "<a href='produk2.php?target=wanita2&page=".($noPage+1)."' id='next'>Next</a>";
		?>
		</div>